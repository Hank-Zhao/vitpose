...
import numpy as np
from dataset import my_coco_dataset
...
def create_model(device):
    model_cfg = {
        ...
        'keypoint_head': {
            'in_channels': 768,
            'out_channels': 17,  # Assuming 17 keypoints for human pose
            'num_deconv_layers': 2,
            'num_deconv_filters': (256, 256),
            'num_deconv_kernels': (4, 4),
            'extra': {'final_conv_kernel': 1},
            'loss_keypoint': {'type': 'JointsMSELoss', 'use_target_weight': True}
        }
    }
...
def main(args):
    ...
    train_dataset = my_coco_dataset.CocoKeypoint(data_root, "train", transforms=data_transform["train"], fixed_size=args.fixed_size, repeats=1)
    val_dataset = my_coco_dataset.CocoKeypoint(data_root, "val", transforms=data_transform["val"], fixed_size=args.fixed_size, det_json_path=args.person_det, repeats=1)
    ...
    input_tensor = torch.randn(64, 3, 256, 192).to(device)
    # Get heatmap output
    heatmap = model(input_tensor)
    print(heatmap.shape)  # Should output something like (1, 17, H, W) where H, W depend on deconv layers
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        coco_info = utils.evaluate(model, val_data_loader, device=device,
                                   flip=False, flip_pairs=person_kps_info["flip_pairs"])
...
if __name__ == "__main__":
    ...
    # COCO数据集人体关键点信息
    parser.add_argument('--keypoints-path', default=r"vitpose\\dataset\\person_keypoints.json", type=str,
